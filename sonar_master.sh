...
hardware-diagnostic	H2testw	Teste l'authenticite et la fiabilite d'une cle USB/carte SD (ecrit puis relit des donnees de test) — detecte les fausses capacites annoncees et les supports defaillants, cas frequent en depannage terrain (client apportant une cle suspecte). Freeware, developpe par le magazine c't (heise.de), aucun compte requis, pas d'installateur (juste l'exe). F3 (Linux/Mac) est l'equivalent mais n'a pas de binaire Windows officiel pret a l'emploi — H2testw reste la reference sous Windows malgre sa derniere version datant de 2008 (toujours fonctionnel sous Windows 10/11).
...
SONAR_VERSION="3.55.0-winpe-tools-clone-drweb"
