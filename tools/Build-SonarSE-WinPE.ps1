...
if ($AddRepairMenu) {
    ...
    $menuLines = @(
        ...
        "echo 10. Diagnostic reseau (ipconfig, ping, reinitialisation Winsock/TCP-IP)"
        ...
        "echo."
        "rem Reinitialisation de la pile reseau : binaires natifs de WinPE (netsh, ipconfig), aucun"
        "rem outil tiers necessaire - couvre le meme besoin qu'un utilitaire externe de type 'NetAdapter"
        "rem Repair' sans en avoir la provenance douteuse (evalue et ecarte cette session)."
        "set netrst="
        "set /p netrst=Reinitialiser la pile reseau (Winsock/TCP-IP, adaptateur bloque) ? (o/n) : "
        "if /i `"%netrst%`"==`"o`" (echo. & echo Reinitialisation en cours... & netsh winsock reset & netsh int ip reset & ipconfig /release & ipconfig /renew & ipconfig /flushdns & echo. & echo Termine - un redemarrage ^(option 0^) peut etre necessaire pour Winsock.)"
        "echo."
        "pause"
        "goto menu"
        ""
        ...
        "rem certutil.exe absent de cette image (verifie en VM, 2026-09-23) : Get-FileHash (PowerShell, module"
        "rem Utility integre, PAS le module TrustedPlatformModule manquant qui bloquait Get-Tpm en option 22)"
        "rem est utilise a la place. Si PowerShell est absent, ECHEC FERME (acces refuse, redemarrage) plutot"
        "rem que de laisser passer sans verification possible - une grille configuree doit refuser, pas s'ouvrir,"
        "rem quand elle ne peut pas verifier."
        "call :findkey"
        "if not defined KEY exit /b 0"
        "if not exist `"%KEY%\MANIFEST\FIELD_PINS.tsv`" exit /b 0"
        "if not exist `"%WINDIR%\System32\WindowsPowerShell\v1.0\powershell.exe`" (echo. & echo ATTENTION : grille PIN configuree sur cette cle mais PowerShell absent de cette image WinPE ^(necessaire pour verifier le PIN^) - acces refuse par securite plutot que laisse ouvert. & echo. & pause & wpeutil reboot)"
        "rem Journal d'acces minimal (Field-Logs\winpe_access.log sur la cle) : cote Linux, chaque acces est"
        "rem deja trace dans audit.log - cote WinPE, rien n'existait avant cette session. Pas un hashchain"
        "rem complet comme cote Linux (hors de portee du cmd.exe pur), juste une trace horodatee de qui/quand."
        "set PINTS=horodatage_indisponible"
        "for /f %%t in ('`"%SB%\busybox.exe`" date +%%Y-%%m-%%d_%%H:%%M:%%S') do set PINTS=%%t"
        "set PINTRY=1"
        ":pin_try"
        "set PINVAL="
        ...
        "del /q X:\sonar_niveau.txt X:\sonar_profils.txt >nul 2>nul"
        "rem Hachage ET recherche de la ligne correspondante faits EN UNE SEULE fois par PowerShell, plutot"
        "rem que hachage en PowerShell puis comparaison texte par cmd.exe (for /f + if) : la comparaison"
        "rem cmd.exe s'est averee ne PAS matcher deux chaines de 64 caracteres pourtant identiques - voir"
        "rem CHANGELOG.md 2026-09-23 pour le detail des deux causes racines trouvees et corrigees (bug"
        "rem d'interpolation `$-au-build sur une premiere reecriture, puis celui-ci : -NoNewline sur les"
        "rem fichiers niveau/profils empechait 'set /p VAR=<fichier' de lire quoi que ce soit, cmd.exe"
        "rem exigeant un terminateur de ligne pour reconnaitre une ligne a lire par redirection). PowerShell"
        "rem ecrit deux fichiers texte simples (niveau, profils) UNIQUEMENT si une ligne correspond - aucun"
        "rem des deux n'existe sinon, ce qui suffit a garder le comportement ferme (echec de hachage/lecture"
        "rem => rien a lire => acces refuse)."
        "powershell -NoProfile -Command `"`$h=(Get-FileHash -Algorithm SHA256 -LiteralPath 'X:\sonar_pin.tmp').Hash.ToLower(); Get-Content -LiteralPath '%KEY%\MANIFEST\FIELD_PINS.tsv' | ForEach-Object { `$f = `$_.Split([char]9); if (`$f.Length -ge 3 -and `$f[1] -eq `$h) { `$f[0] | Out-File X:\sonar_niveau.txt -Encoding ascii; `$f[2] | Out-File X:\sonar_profils.txt -Encoding ascii } }`" 2>nul"
        "del /q X:\sonar_pin.tmp >nul 2>nul"
        "set NIVEAU="
        "set PROFILS="
        "if exist X:\sonar_niveau.txt set /p NIVEAU=<X:\sonar_niveau.txt"
        "if exist X:\sonar_profils.txt set /p PROFILS=<X:\sonar_profils.txt"
        "del /q X:\sonar_niveau.txt X:\sonar_profils.txt >nul 2>nul"
        "rem Bloc a une ligne 'if defined NIVEAU (plusieurs & imbriques)' deliberement evite ici : NIVEAU"
        "rem se retrouvait prouve non-vide juste avant (echo DEBUG) mais la branche ACCES_ACCORDE n'etait"
        "rem quand meme jamais executee, cause jamais identifiee avec certitude malgre plusieurs heures de"
        "rem diagnostic instrumente cette session (voir CHANGELOG.md 2026-09-23) - remplace par des lignes"
        "rem simples (une commande par ligne, comme le reste de ce fichier) plutot que traque plus loin."
        "if not defined NIVEAU goto pin_bad"
        "set SONAR_NIVEAU=%NIVEAU%"
        "if not exist `"%KEY%\Field-Logs`" mkdir `"%KEY%\Field-Logs`""
        ">>`"%KEY%\Field-Logs\winpe_access.log`" echo %PINTS%	ACCES_ACCORDE	niveau=%NIVEAU%"
        "echo."
        "echo Acces accorde - niveau : %NIVEAU% (profils : %PROFILS%)"
        "pause"
        "exit /b 0"
        ":pin_bad"
        "echo PIN incorrect."
        ...
        "if not exist `"%KEY%\Field-Logs`" mkdir `"%KEY%\Field-Logs`""
        ">>`"%KEY%\Field-Logs\winpe_access.log`" echo %PINTS%	VERROUILLAGE	3_tentatives_incorrectes"
        ...
        "set espvol="
        "set /p espvol=Numero du volume ESP (Entree = annuler, ou C = aucune ESP visible, en creer une) : "
        "if `"%espvol%`"==`"`" goto menu"
        "if /i `"%espvol%`"==`"C`" goto uefi_create_esp"
        ...
        ""
        ":uefi_create_esp"
        "rem Cas non couvert avant cette session : bootrec/bcdboot supposent tous deux une ESP EXISTANTE"
        "rem a reparer, aucun ne sait en CREER une absente (constat fait sur le HP EliteBook de test, ou"
        "rem le diagnostic intelligent avait detecte 'aucune partition systeme EFI'). Necessite de l'espace"
        "rem non alloue sur le disque - si le disque est plein, il faut d'abord en liberer (option 3)."
        "cls"
        "echo --- Creation d'une nouvelle partition ESP ---"
        "echo ATTENTION : necessite de l'espace NON ALLOUE sur le disque cible (100 Mo minimum)."
        "echo Si le disque est plein, annulez et liberez de l'espace d'abord (option 3, diskpart)."
        "echo."
        "echo list disk> X:\sonar_dp5.txt"
        "diskpart /s X:\sonar_dp5.txt"
        "echo."
        "set espdisk="
        "set /p espdisk=Numero du disque cible (Entree = annuler) : "
        "if `"%espdisk%`"==`"`" goto menu"
        "set go2="
        "set /p go2=Creer une ESP FAT32 de 100 Mo sur le disque %espdisk% et y installer le demarrage depuis %lettre%:\Windows ? (o/n) : "
        "if /i not `"%go2%`"==`"o`" goto menu"
        ">X:\sonar_dp6.txt echo select disk %espdisk%"
        ">>X:\sonar_dp6.txt echo create partition efi size=100"
        ">>X:\sonar_dp6.txt echo format quick fs=fat32 label=`"System`""
        ">>X:\sonar_dp6.txt echo assign letter=S"
        "diskpart /s X:\sonar_dp6.txt"
        "if not exist S:\ (echo Creation de la partition impossible - verifiez qu'il y a bien de l'espace non alloue sur ce disque. & pause & goto menu)"
        "bcdboot %lettre%:\Windows /s S: /f UEFI"
        ">X:\sonar_dp7.txt echo select volume S"
        ">>X:\sonar_dp7.txt echo remove letter=S"
        "diskpart /s X:\sonar_dp7.txt >nul"
        "echo."
        "echo Termine. Redemarrez (option 0) et testez le demarrage."
        "pause"
        "goto menu"
