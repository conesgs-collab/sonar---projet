...
if ($AddRepairMenu) {
    ...
    $menuLines = @(
        ...
        "powershell -NoProfile -Command `"(Get-FileHash -Algorithm SHA256 -LiteralPath 'X:\sonar_pin.tmp').Hash.ToLower()`" > X:\sonar_pin_hash.txt 2>nul"
        "set /p PINHASH=<X:\sonar_pin_hash.txt"
        "del /q X:\sonar_pin.tmp X:\sonar_pin_hash.txt >nul 2>nul"
        "rem Garde-fou : un PINHASH vide (echec de hachage) ne doit JAMAIS pouvoir matcher une colonne vide -"
        "rem sans ce controle explicite, un bug de lecture du fichier (voir 'usebackq' ci-dessous) peut faire"
        "rem correspondre deux chaines vides et accorder un acces non verifie (trouve en VM, 2026-09-23)."
        "if not defined PINHASH goto pin_bad"
        "set NIVEAU="
        "set PROFILS="
        "rem 'usebackq' est OBLIGATOIRE ici : sans lui, un nom de fichier entre guillemets doubles dans for /f"
        "rem est traite comme une CHAINE LITTERALE A PARSER (pas comme un fichier a ouvrir) - le fichier n'etait"
        "rem alors jamais lu, %%b restait vide, et vide==vide accordait l'acces a n'importe quel PIN (meme bug"
        "rem trouve en meme temps que le certutil manquant, VM, 2026-09-23)."
        "for /f `"usebackq tokens=1,2,3 delims=`t`" %%a in (`"%KEY%\MANIFEST\FIELD_PINS.tsv`") do if /i `"%%b`"==`"%PINHASH%`" (set NIVEAU=%%a & set PROFILS=%%c)"
        "if defined NIVEAU (set SONAR_NIVEAU=%NIVEAU% & if not exist `"%KEY%\Field-Logs`" mkdir `"%KEY%\Field-Logs`" & >>`"%KEY%\Field-Logs\winpe_access.log`" echo %PINTS%	ACCES_ACCORDE	niveau=%NIVEAU% & echo. & echo Acces accorde - niveau : %NIVEAU% ^(profils : %PROFILS%^) & pause & exit /b 0)"
        ":pin_bad"
        "echo PIN incorrect."
        "set /a PINTRY+=1"
